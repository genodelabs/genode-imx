...
class Root : public Root_component<genode_block_session>
{
	...
	public:
		...
		void announce_driver(const char * name, Block::Session::Info info);
...
void ::Root::announce_driver(const char * name, Block::Session::Info info)
{
	for (unsigned idx = 0; idx < MAX_BLOCK_DEVICES; idx++) {
		if (_sessions[idx].constructed())
			continue;
                                             ...
                                             int                writeable)
{
	...
	_block_root->announce_driver(name, { 1UL << SIZE_LOG2_512,
